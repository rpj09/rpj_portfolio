...
export function VaporizeTextCycle({
    ...
    const globalDpr = useMemo(() => {
        if (typeof window !== "undefined") {
            return Math.min(window.devicePixelRatio, 2); // Capped at 2x, was 1.5 * dpr = 3x on Retina!
        }
        return 1;
    }, []);
...
const createParticles = (
    ...
    // Calculate sampling rate based on target maximum particles (~12,000) to ensure buttery 60fps scroll
    const currentDPR = canvas.width / parseInt(canvas.style.width || "1000");

    // Estimate text pixel coverage (roughly 30% of the bounding box area)
    const fontSize = parseInt(font.match(/\d+/)?.[0] || "50");
    const estimatedTextArea = textWidth * fontSize;
    const estimatedPixels = estimatedTextArea * currentDPR * currentDPR * 0.3;

    // Target 12,000 particles max
    let sampleRate = Math.max(2, Math.ceil(Math.sqrt(estimatedPixels / 12000)));
    ...
    console.log(`Vaporize: Created ${particles.length} particles for text "${text}" with color ${color} and font ${font} on canvas ${canvas.width}x${canvas.height}`);
...
// ------------------------------------------------------------ //
const calculateVaporizeSpread = (fontSize: number | string) => {
